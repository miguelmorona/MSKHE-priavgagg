...
var benchParameters = []parameters{
	...
	//----------------------------------------------------------------------------------------------//
	{NumParties: 256, n: 4, PreComputeA: false, logN: 11, logQ: [2]int{2, 21}, plevel: 0},
	//----------------------------------------------------------------------------------------------//
}
...
func main() {
	...
	for _, param := range paramsSets {
		...
		for z := 0; z < nexperiments; z++ {
			...
			encInput, partialDec := encPhase(priaggrings, gaussianSamplerQ, uniformSamplerQ, n, P, param)
			l.Printf("\t\t\t||                                                                                                                       ||")
			...
			l.Printf("\t\t\t||           - Plaintext space = %15d                                                                         ||", ringQ.ModulusAtLevel[param.plevel])
			...
			for i := range flatAggExp {
				...
				if i < 65 || i >= (len(flatAggExp))-65 {
					l.Printf("\t\t\t||   Row [%8d] of Aggregated Model w.     Exp Aggregation Result: %15d,     Decrypted: %15d   ||", i+1, flatAggExp[i], flatRecAggShare[i])
				}
			...
			if maxAbsErr > relErrorThreshold {
				l.Printf("\t\t\t||                                                                                      ||===============================||")
				l.Printf("\t\t\t||                                                                                      || Number of errors: %5d       ||", nerror)
				l.Printf("\t\t\t||                                                                                      ||===============================||")
				l.Printf("\t\t\t||=======================================================================================================================||")
				l.Printf("\t\t\t||                                            ❌ ❌  INCORRECT :((                                                       ||")
				l.Printf("\t\t\t||=======================================================================================================================||")				
...
func decPhase(aggring *AggRings, partialDec [][]ring.Poly, encShareAgg []ring.Poly, n int, param parameters) (recShare []ring.Poly) {
	...
	elapsedDecCloud += runTimed(func() {
		...
		for j := 0; j < n; j++ { // running through ciphertexts per party
			...
			// 1. Reduce level and resize
			aggring.ringQ.AtLevel(recShare[j].Level()).DivRoundByLastModulusManyNTT(recShare[j].Level()-param.plevel, recShare[j], buff, recShare[j])
