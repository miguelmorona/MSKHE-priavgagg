...
// ----------------------------------------------------------------------------------------------//
// BFV-MHE.go can be run as: go run ./BFV-MHE.go NumParties Goroutines NumModelParameters
// - By default: NumParties = 32, Goroutines = 1, NumModelParameters = 16384000
//
// Cryptosystem parameters are defined in "paramsDef"
// - By default:
//	paramsDef := bgv.ParametersLiteral{
//		LogN:             13,
//		LogQ:             []int{60, 60}, // 120 BITS
//		LogP:             []int{31},
//		PlaintextModulus: 1785857, // 109*2^14 + 1 (21 bits)
//	}
//
// ----------------------------------------------------------------------------------------------//
// Comments: The code of this script was started by relying on the Examples folder available in Lattigo "https://github.com/tuneinsight/lattigo" and the BFVbaselineAgg folder in "https://github.com/apedrouzoulloa/mkagg"
// ----------------------------------------------------------------------------------------------//
...
// go run ./BFV-MHE.go 16 1 16384000

func main() {	

	var err error

	file, err := os.Create("bfv-results-flta.txt")
	check(err)
	defer file.Close() 
	l := log.New(file, "", 0)
	...
	for z := 0; z < nexperiments; z++ {
		...
		params, err := bgv.NewParametersFromLiteral(paramsDef)
		check(err)
		...
		l.Printf("\tCKG done (cloud: %s, party: %s)\n", elapsedCKGCloud, elapsedCKGParty)
		...
		for i := range encOut {
			ptres[i] = bgv.NewPlaintext(params, params.MaxLevel())
		}
		...
		// Decode phase: convert plaintext type into int.
...
func genparties(params bgv.Parameters, NumParties int) []*party {
...
func genInputs(params bgv.Parameters, P []*party, NumModelParameters int, BoundInputs uint64) (expRes []uint64) {
...
func ckgphase(params bgv.Parameters, crs sampling.PRNG, P []*party, l *log.Logger) *rlwe.PublicKey {
...
func encPhase(params bgv.Parameters, P []*party, pk *rlwe.PublicKey, encoder *bgv.Encoder, l *log.Logger) (encInputs [][]*rlwe.Ciphertext) {
...
func evalPhase(params bgv.Parameters, NGoRoutine int, encInputs [][]*rlwe.Ciphertext, l *log.Logger) (encRes []*rlwe.Ciphertext) {
...
func pcksPhase(params bgv.Parameters, tpk *rlwe.PublicKey, encRes []*rlwe.Ciphertext, P []*party, l *log.Logger) (encOut []*rlwe.Ciphertext) {
